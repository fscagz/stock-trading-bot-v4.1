"""
Conditional news filter backtest — all three periods.

Configs (all dynamic equity, live-bot settings):
  1. No news filter
  2. News always required
  3. Conditional: news required only when SPY 10-session return <= 0
                  (bear/stalling regime); no filter when SPY momentum is positive
"""
...
def spy_10session_return(d: date) -> float:
    """Return SPY's % change over the 10 sessions before d."""
    df = _load_spy()
    past = df[df.index < pd.Timestamp(d)]
    if len(past) < 11:
        return 0.0
    tail = past["close"].iloc[-11:]
...
def run(
    ...
    for d in days:
        ...
        if conditional:
            bear_regime = spy_10session_return(d) <= 0
            active_nf   = news_filter if bear_regime else None
            active_mode = "require"   if bear_regime else "ignore"
        else:
...
def main():
    ...
    hdr = (f"  {'Config':<38} {'Trades':>7} {'WR':>7} {'Avg W':>8} {'Avg L':>8} "
    ...
    for period_label, (start, end) in PERIODS.items():
        ...
        configs = [
            ("No news filter",                    None,        False),
            ("News always required",              news_filter, False),
            ("News when SPY 10d return <= 0",     news_filter, True),
        ]

        for label, nf, cond in configs:
            m = run(copy.copy(cfg), days, screener, fetcher, initial_equity, nf, conditional=cond)
            print(
                f"  {label:<38} {m['total_trades']:>7,} {m['win_rate']:>6.1%} "
