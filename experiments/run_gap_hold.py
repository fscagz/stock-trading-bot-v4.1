"""
Gap-and-hold new entry signal.

Tests whether adding a gap-and-hold entry path improves trade count and
performance. Signal logic:

  1. Stock gaps up >= gap_min_pct at the open vs. prior close.
  2. Over the next 15 bars (15 min), price holds within gap_tolerance of
     the day open — gap not filled/broken.
  3. On bar 16: enter at market with standard ATR-based sizing.
     News catalyst still required (same gate as momentum entries).

This is additive to the existing H momentum signal — both paths can fire
on different symbols on the same day. traded_today prevents double entry
on the same symbol.

Gap thresholds tested: 3%, 5%, 7% minimum gap at open.
Baseline: H config without any gap-hold (momentum only).
"""
...
# (label, gap_hold_on, gap_min_pct)
CONFIGS = [
    ("Baseline (momentum only)", False, 0.0),
    ("+ gap-hold ≥3%",           True,  0.03),
    ("+ gap-hold ≥5%",           True,  0.05),
    ("+ gap-hold ≥7%",           True,  0.07),
]
...
def main():
    ...
    print(f"H config, $10M DV — gap-and-hold signal sweep")
    print(f"Hold params: {GAP_HOLD_BARS} bars, ≤{GAP_HOLD_TOLERANCE:.0%} retracement\n")

    fetcher     = BarFetcher(api_key, secret_key)
    news_filter = NewsFilter(api_key, secret_key, cache_only=False)

    HDR = (f"  {'Config':<26} {'Trades':>6} {'Mom':>5} {'Gap':>5} "
    ...
    for period_label, start, end, spy_ret in PERIODS:
        ...
        for label, gap_on, gap_pct in CONFIGS:
            cfg = copy.copy(base)
            ...
            print(f"  {label:<26} {m['trades']:>6,} {m['mom_trades']:>5} {m['gap_trades']:>5} "
