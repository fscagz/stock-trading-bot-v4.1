"""
Entry filter sweep: VWAP filter and bar-range spread proxy.

Tests two additional entry gates against the validated H baseline
(hybrid tier-4, $10M DV, no MA filter) across all 5 backtest periods.

VWAP filter:
    Skip entry when bar.close <= intraday VWAP at signal time.
    Hypothesis: entries above VWAP have buying pressure confirmation;
    entries below VWAP are fading into a downtrend within the day.

Bar-range spread proxy:
    Skip entry when (bar.high - bar.low) / bar.close > threshold.
    Wide intraday bars imply wide effective spread and/or illiquid tick
    structure. This is a proxy — we don't have NBBO data in the backtest
    cache, so treat evidence here as suggestive rather than definitive.
    Thresholds tested: none, ≤1%, ≤2%, ≤3%, ≤5%.
"""
...
SPREAD_THRESHOLDS = [0.0, 0.01, 0.02, 0.03, 0.05]
...
def run_period(cfg, start, end, screener, fetcher, initial_equity, news_filter,
               vwap_filter: bool, spread_pct: float) -> dict:
    ...
    sim = Simulator(
        ...
        require_above_vwap_at_entry=vwap_filter,
        max_entry_bar_range_pct=spread_pct,
    )
...
def main():
    ...
    print(f"Config: H (hybrid tier-4), $10M DV, 5 periods\n")

    fetcher     = BarFetcher(api_key, secret_key)
    news_filter = NewsFilter(api_key, secret_key, cache_only=False)

    HDR = (f"  {'Config':<24} {'Trades':>6} {'WR':>6} {'Return':>8} "
           f"{'MaxDD':>7} {'vs S&P':>8} {'PF':>5} {'HStop':>6} {'Tgts':>5} "
           f"{'Chase':>6} {'PB':>5}")
    SEP = "  " + "-" * (len(HDR) - 2)
    ...
    for period_label, start, end, spy_ret in PERIODS:
        ...
        # ── Section 1: VWAP filter ──────────────────────────────────────────
        print(f"\n  ── VWAP filter ──")
        print(HDR); print(SEP)
        for vwap_on, label in [(False, "Baseline (no VWAP)"), (True, "VWAP filter on")]:
            cfg = copy.copy(base)
            m = run_period(cfg, start, end, screener, fetcher, initial_equity,
                           news_filter, vwap_filter=vwap_on, spread_pct=0.0)
            print_row(label, m, spy_ret)

        # ── Section 2: Bar-range spread proxy ───────────────────────────────
        print(f"\n  ── Bar-range spread proxy (bar H-L / close)  [caveat: proxy, not NBBO] ──")
        print(HDR); print(SEP)
        labels = {0.0: "Baseline (none)", 0.01: "≤1%", 0.02: "≤2%",
                  0.03: "≤3%", 0.05: "≤5%"}
        for thr in SPREAD_THRESHOLDS:
            cfg = copy.copy(base)
            m = run_period(cfg, start, end, screener, fetcher, initial_equity,
                           news_filter, vwap_filter=False, spread_pct=thr)
            print_row(labels[thr], m, spy_ret)
