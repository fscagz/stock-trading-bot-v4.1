"""
Minimum liquidity filter sweep — Experiment 1.

Tests whether restricting the candidate universe to stocks above a minimum
average daily dollar volume reduces microcap noise without materially
degrading the large-cap momentum trades that drive the 2025-26 returns.

The filter is applied at the screener level (candidate selection) rather than
the simulator level, so it mirrors what a live scanner would see.

Thresholds tested:
    $500k  (current baseline — make_gap_hold_config default)
    $5M
    $10M
    $20M
    $50M

Config: H+ (hybrid tier-4 + MA10 narrow-day filter), same as the finalist run.
"""
...
# (label, min_avg_dollar_volume in dollars)
THRESHOLDS = [
...
def run_period(cfg, start, end, screener, fetcher, initial_equity, news_filter) -> dict:
...
def make_screener(base_cfg, base_screener, dv_threshold, api_key, secret_key,
                  base_url, start, end):
    """Build a screener sharing base_screener's daily cache but with a new DV threshold."""
    cfg = copy.copy(base_cfg)
    cfg.min_avg_dollar_volume = dv_threshold
    screener = CandidateScreener(cfg, api_key, secret_key, base_url)
    screener._universe = base_screener._universe
    screener._daily_cache = base_screener._daily_cache   # shared read-only reference
...
def main():
    ...
    print(f"H+ config (hybrid tier-4 + MA10 narrow-day filter, DV cap 20%)")
    print(f"Sweeping min average daily dollar volume thresholds\n")
    ...
    for period_label, start, end, spy_ret in PERIODS:
        # Preload once with the baseline config (daily cache is the same for all thresholds)
        base = make_gap_hold_config()
        base.stage2_min_vol_vs_prev_bar = 0.80
        base.target_atr_multiple = CHASE_TARGET
        base.min_avg_dollar_volume = THRESHOLDS[0][1]  # $500k baseline
        ...
        print(f"{'='*84}")
        print(f"  {'Min DV':<20} {'Trades':>6} {'WR':>6} {'Return':>8} "
              f"{'MaxDD':>7} {'vs S&P':>8} {'Chase':>6} {'PB':>5}")
        print("  " + "-" * 72)

        for label, dv in THRESHOLDS:
            if dv == THRESHOLDS[0][1]:
                screener = base_screener
            else:
                screener = make_screener(base, base_screener, dv,
                                         api_key, secret_key, base_url, start, end)
            cfg = copy.copy(base)
            cfg.min_avg_dollar_volume = dv
            m = run_period(cfg, start, end, screener, fetcher, initial_equity, news_filter)
