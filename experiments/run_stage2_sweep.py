...
Tests whether the current stage2 entry thresholds are too restrictive.
Sweeps relative volume (10× → 6×) and buying pressure (0.85 → 0.75)
on the validated H config ($10M DV) across all 5 backtest periods.

Hypothesis: at $10M DV the universe is already high-quality; the strict
thresholds filter signal, not noise, leaving the strategy undertrades.
"""
...
# (label, stage2_min_relative_volume, stage2_buying_pressure_min)
CONFIGS = [
    ("Baseline  (rv=10, bp=0.85)", 10.0, 0.85),
    ("rv=8,  bp=0.85",              8.0, 0.85),
    ("rv=6,  bp=0.85",              6.0, 0.85),
    ("rv=10, bp=0.80",             10.0, 0.80),
    ("rv=10, bp=0.75",             10.0, 0.75),
    ("rv=8,  bp=0.80",              8.0, 0.80),
    ("rv=6,  bp=0.75",              6.0, 0.75),
]
...
def run_period(cfg, start, end, screener, fetcher, initial_equity, news_filter) -> dict:
    ...
    if not trades:
        return {"trades": 0, "wr": 0.0, "ret_pct": 0.0, "max_dd": 0.0,
                "hard_stops": 0, "targets": 0, "profit_factor": 0.0}
...
def main():
    ...
    print(f"H config, $10M DV — stage2 threshold sweep\n")
    ...
    for period_label, start, end, spy_ret in PERIODS:
        ...
        for label, relvol, bp in CONFIGS:
            cfg = copy.copy(base)
            cfg.stage2_min_relative_volume = relvol
            cfg.stage2_buying_pressure_min = bp
            ...
            print(f"  {label:<28} {m['trades']:>6,} {m['wr']:>5.1f}% "
                  ...
                  f"{pf:>5} {m['hard_stops']:>6} {m['targets']:>5}", flush=True)
        print()
