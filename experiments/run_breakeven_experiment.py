"""
Break-even stop experiment — all three periods.

Sweeps breakeven_trigger_atr_multiple: 0 (off) / 0.25 / 0.5 / 1.0
and also combines with tighter trailing_stop_atr_multiple: 2.0 (current) / 1.0
...
def run_config(
    ...
    trades = all_trades
    if not trades:
        return {"trades": 0, "wr": 0.0, "ret_pct": 0.0, "hard_stops": 0, "be_stops": 0, "targets": 0}

    wins    = [t for t in trades if t.pnl and t.pnl > 0]
    stops   = [t for t in trades if t.exit_reason == "hard_stop"]
    be_hits = [t for t in trades if t.exit_reason == "trailing_stop"]
    ...
    return {
        ...
        "hs_avg":     sum(t.pnl for t in stops if t.pnl) / len(stops) if stops else 0.0,
        "be_stops":   len(be_hits),
        "targets":    len(tgts),
    }
...
def main():
    ...
    print(f"Base: top-50 cap, day_high_5pct, target=4×ATR, tier-4 bypass\n")

    fetcher     = BarFetcher(api_key, secret_key)
    news_filter = NewsFilter(api_key, secret_key, cache_only=False)

    # (label, breakeven_trigger, trailing_atr)
    experiments = [
        ("No BE stop  trail=2.0×", 0.0,  2.0),
        ("BE=0.25 ATR trail=2.0×", 0.25, 2.0),
        ("BE=0.50 ATR trail=2.0×", 0.50, 2.0),
        ("BE=1.00 ATR trail=2.0×", 1.00, 2.0),
        ("No BE stop  trail=1.0×", 0.0,  1.0),
        ("BE=0.25 ATR trail=1.0×", 0.25, 1.0),
        ("BE=0.50 ATR trail=1.0×", 0.50, 1.0),
        ("BE=1.00 ATR trail=1.0×", 1.00, 1.0),
    ]

    for period_label, start, end, spy_ret in PERIODS:
        base_cfg = make_gap_hold_config()
        base_cfg.target_atr_multiple = 4.0
        base_cfg.stage2_min_dist_from_day_high_pct = 0.05
        ...
        print(f"  {len(screener._daily_cache):,} symbols\n", flush=True)
        ...
        print(f"{'='*80}")
        hdr = (f"  {'Config':<26} {'Trades':>7} {'WR':>6} {'Return':>8} {'vs S&P':>8} "
               f"{'HStops':>7} {'HS Avg':>9} {'BE hits':>8} {'Targets':>8}")
        print(hdr)
        print("  " + "-" * 84)

        for label, be_trig, trail in experiments:
            cfg = copy.copy(base_cfg)
            cfg.breakeven_trigger_atr_multiple = be_trig
            cfg.trailing_stop_atr_multiple     = trail
            ...
            print(
                ...
                f"{m['hard_stops']:>7} ${m['hs_avg']:>8,.0f} "
                f"{m['be_stops']:>8} {m['targets']:>8}",
                flush=True,
            )
        print()
